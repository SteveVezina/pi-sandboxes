...
### T3.1: Namespace setup ✅ *(2026-07-14: re-verified per PROP-008 — build fix + real availability validation landed)*
...
**Acceptance criteria:**
- [x] `syscall.SysProcIDRange` replaced with `syscall.SysProcIDMap` — Linux build compiles
- [x] `fast.Validate()` actually executes its probe command and reports real errors (no false-positive availability when unprivileged user namespaces are disabled)
...
**Verification:**
- [x] `GOOS=linux go build ./pkg/runtime/fast/...` succeeds
- [x] Unit test: `TestValidate_MatchesHostCapability` — Validate() result must match a direct namespace clone probe (fails when user namespaces are disabled)
