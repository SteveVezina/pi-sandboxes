...
**No ⚠️ Needs re-verify features remain.** Open work: (1) F30 T30.4/T30.3-firewall/T30.8-HTTPS — Linux host or larger design; (2) F9 archive size cap — needs a block-spec default (no PROP allowed while PROP-006 open unless BLOCKING; it isn't); (3) F13 cache-scoping fix (needs small PROP/ADR); (4) F29 Agent Run implementation (deps now unblocked: F9 ✅, F30 core ✅, ADR-007 events ✅); (5) F18 gVisor driver — broken on Linux, needs Linux host.
