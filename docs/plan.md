...
**F30 remaining:** T30.4 (L3 isolation, Linux-blocked), T30.3 compat firewall (Linux), T30.8 HTTPS credential-helper channel. All either Linux-host or larger design. Core egress path (policy → proxy → allowlist → denial log → HTTP credential injection) is done and tested on darwin.
