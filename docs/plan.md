...
**F28 T28.4 (GUI templates) — done (2026-08-31):** `apps/gui` `TemplatesView` is now a full management surface — list, per-template detail (source/lineage, digest+generation, image, network, declared runtime compat), validation problems, revision history + per-revision rollback, diff (template or `name@N`), fork, validate, set-default, export (download), import (file upload). `apps/gui/src/api.ts` template methods. Only the snapshot-from-sandbox action is deferred (T28.2b, Linux). AC-35.12 ✅.

**F28 status: T28.1–T28.4 done. Only T28.2b (snapshot from a sandbox — needs runtime hooks on Linux) remains.**
