...
**Next work (2026-08-31):** F30 T30.1/T30.2/T30.5/T30.6/T30.7 done, T30.3 partial. Checkpoint review **waived by user** ("skip validation for now"). T30.7: `POST/GET /v1/credentials`, `secrets.CredentialStore` in-memory values, `secrets.ValueSource` (literal/env/file, file-under-~/.pi-box rejected; keychain stubbed). Also fixed a latent slice-index panic in `secrets.CredentialStore.GetForHost`. 480 tests green. NEXT: T30.8 (resolve credential id→value in the proxy, inject into approved requests: git credential helper channel + registry Authorization header; replace the `"[credential-injected]"` placeholder). Still Linux-blocked: T30.4.
