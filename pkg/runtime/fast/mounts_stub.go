...
func (c *MountConfig) EnsureDirectories() error {
	dirs := []string{c.Rootfs, c.Workspace, c.Artifacts, c.Tmp, c.Home}
	for _, d := range dirs {
		if err := os.MkdirAll(d, 0755); err != nil {
			return fmt.Errorf("create dir %s: %w", d, err)
		}
	}
	for _, d := range c.Caches {
		if err := os.MkdirAll(d, 0755); err != nil {
			return fmt.Errorf("create cache dir %s: %w", d, err)
		}
	}
	return nil
}
...
func (c *MountConfig) Validate() error {
	if len(c.HostMounts) > 0 {
		return fmt.Errorf("host mounts are not allowed by default")
	}
	return nil
}
